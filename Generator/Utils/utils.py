...
def levenshtein(a,b):
    ...
    for i in range(1,m+1):
        ...
        for j in range(1,n+1):
            add, delete = previous[j-1]+1, current[j-1]+1
            change = previous[j-1]
            if a[j-1] != b[i-1]:
                change = change + 1
...
def annotated_to_kern(annotated_str: str) -> str:
    """
    Converts PRAIG-annotated strings back into standard Humdrum **kern syntax 
    so that rendering engines like verovio can use it directly.
    """
    krn = annotated_str.replace("**ekern", "**kern")
    krn = krn.replace("@", "")
    krn = krn.replace("·", "")
    return krn
